...
class AbstractType {
  ...
  constructor(name, requiredKeys = [], optionalKeys = [], defaultValues = {}) {
    ...
    this.optionalKeys = Immutable.Set.of('cache', 'arguments', 'tags').union(optionalKeys);
   ...
   */
  validate(mapping) {
    const mappingKeys = mapping.keySeq().toSet();
    const extraKeys = mappingKeys.subtract(this.possibleKeys);
    const missingKeys = this.requiredKeys.subtract(mappingKeys);
  ...
  createObject(mapping, resolveArgument) {
    try {
      const objectCreator = this._resolveObjectCreator(mapping);
      const args = this._resolveArguments(mapping, resolveArgument);
      const createdObject = this._doCreateObject(objectCreator, args, mapping);
